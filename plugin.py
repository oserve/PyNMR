...
def alterBFactors(pdb,bFactor):
    cmd.alter(pdb,"b="+ str(bFactor))

def spectrum(color_gradient,pdb):
    cmd.spectrum("b",color_gradient,pdb)
...
def drawConstraint(points, color, aRadius, ID):
	...
	cmd.load_cgo(cons, ID)

def delete(selectionName):
    cmd.delete(selectionName)
...
class NMRGUI(Tk.Tk):
	...
	def createPanels(self):
		#Main Frames (not IBM)
		self.constraintSelectionManagement = ConstraintSelectionPanel(self)
		self.panelsList.append(self.constraintSelectionManagement)

		self.constraintFilesManagement = FileSelectionPanel(self)
		self.panelsList.append(self.constraintFilesManagement)

		self.NOEDrawingManagement = NOEDrawingPanel(self)
		self.NOEDrawingManagement.grid(row=0, column=2)
		self.panelsList.append(self.NOEDrawingManagement)

		self.preferencesPanel=PreferencesPanel(self)
...
cmd.extend("loadNOE", loadNOE)
cmd.extend("showNOE", showNOE)
cmd.extend("showNOEDensity", showNOEDensity)
cmd.extend("loadAndShow", loadAndShow)
cmd.extend("cleanScreen", cleanScreen)
